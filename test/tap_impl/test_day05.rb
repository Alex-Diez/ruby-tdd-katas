...
describe 'tap impl method' do
  ...
  context 'access to instance internals' do
    ...
    before(:each) do
      @ref_to_instance_var = 'ref'
      @cc = CustomClass.new(@ref_to_instance_var)
    end
    
    it 'has access to instance variables' do
      expect { |block| @cc.tap_impl { @instance_var.tap_impl(&block) } }.to yield_with_args(@ref_to_instance_var)
    end

    it 'has access to private methods' do
      expect { |block| @cc.tap_impl { private_method.tap_impl(&block) } }.to yield_with_args(@ref_to_instance_var)
    end
  end
end
